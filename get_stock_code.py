...
def get_stock_code_from_csv():
    stock = []
    try:
        with open("D:\db_backups\stock_code.csv", mode="r") as csv_file:
            csv_reader = csv.DictReader(csv_file)
            for row in csv_reader:
                # 確保 "stock_code" 和 "type" 欄位都存在
                if "stock_code" in row and "type" in row:
                    stock.append({"stock_code": row["stock_code"].strip(), "type": row["type"].strip()})
                else:
